...
<style>
  ...
  *{{box-sizing:border-box}}
  ...
  .mono{{font-family:ui-monospace,Menlo,monospace}}
    ...
    A(f"<h1>{esc(name)}</h1>")
    A(
        f'<div class="sub">Síntesis del descubrimiento — generado {date.today().isoformat()}.</div>'
    )
    A('<div class="counts">')
    for val, lab in [
        (n_interviews, "entrevistas"),
    ...
    A('<div class="legend">')
    A(
        f'<div><span class="dot" style="background:{GREEN}"></span>respaldo: primera mano</div>'
    )
    A(
        f'<div><span class="dot" style="background:{AMBER}"></span>respaldo: referenciada</div>'
    )
    ...
    for p in personas:
        ...
        src_line = (
            f'<div class="sub mono" style="font-size:12px;margin-top:4px">fuente: {esc(src)}</div>'
            if src
            else '<div class="sub mono" style="font-size:12px;margin-top:4px">sin entrevista propia</div>'
        )
        A(
            f'<div class="card persona"><div style="display:flex;justify-content:space-between;align-items:start;gap:8px">'
    ...
    A(
        '<div class="sub" style="margin-bottom:8px">Lo que entregamos solo vale si cambia un comportamiento y mueve un número. Cada flecha es una hipótesis a comprobar.</div>'
    )
    A('<div class="flow">')
    A(
        f'<div class="node" style="background:{BLUE}">OUTPUT<small>Las funcionalidades del MVP (ver mvp-canvas.md)</small></div>'
    )
    A('<div class="arrow">→</div>')
    A(
        f'<div class="node" style="background:{BLUE2}">OUTCOME<small>El cambio de comportamiento esperado en las personas</small></div>'
    )
    A('<div class="arrow">→</div>')
    A(
        f'<div class="node" style="background:{AMBER}">IMPACT<small>El número de negocio que se mueve</small></div>'
    )
    A("</div>")
    ...
    if not hyps:
        A(
            '<div class="sub">Aún no hay experimentos. Corre <span class="mono">/discovery:experiments</span>.</div>'
        )
    for h in hyps:
        ...
        A(
            f'<div class="head"><h3>{esc(h.get("id", ""))} · {esc(h.get("assumption", ""))}</h3>'
    ...
    # ---------- FOOT ----------
    A(
        f'<div class="foot">Generado por <span class="mono">build-report.py</span> desde '
...
def main():
    ...
    if not os.path.isdir(out_dir):
        print(
            f"No existe {out_dir}. ¿Corriste analyze/generate-mvp primero?",
            file=sys.stderr,
        )
        sys.exit(1)
