#!/usr/bin/env python3
"""
readiness-gate.py — Hook PreToolUse genérico para el Discovery Agent.
...
discovery desde la RUTA del archivo que se va a escribir, así que no depende de
...
Solo actúa cuando la escritura apunta a <discovery>/outputs/mvp-canvas.md o
...
QUÉ VALIDA (independiente, no confía en el self-report del modelo)
==================================================================
...
3. Cada persona PRIMARIA está respaldada por una entrevista en PRIMERA PERSONA
   que exista en disco (no basta con que la mencionen en la de otro).
4. Ningún DOLOR HUÉRFANO: cada dolor cita una fuente que existe en disco.
...
def fail(reason_lines):
    ...
    print(border, file=sys.stderr)
    print("Acción: levanta más evidencia (agrega/entrevista) y reintenta.",
          file=sys.stderr)
    print(f"{border}\n", file=sys.stderr)
    sys.exit(2)
...
def main():
    ...
    try:
        ...
        fail([f"  evidence-map.json no es JSON válido: {exc}"])
    ...
    for p in personas:
        ...
        if role and role not in fp_roles:
            problems.append(
                ...
                f"en primera persona en disco. Está construida de oídas."
            )
